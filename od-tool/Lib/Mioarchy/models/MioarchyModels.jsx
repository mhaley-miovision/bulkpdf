...
//
Mioarchy.prototype = {
    Types: {Role: 0, Application: 1, Contributor: 2, RoleLabel: 3, Organization: 4, Accountability: 5},
    ...
    getOrganizationRoles: function (organization, recurse) {
        var list = [];

        // roles at this level
        for (var c in this.roles) {
            // make sure the role is attached to an org
            var roleOrgName = this.roles[c].organization;
            if (roleOrgName) {
                // see if the names match
                if (roleOrgName.toLowerCase() === organization.name.toLowerCase()) {
                    list.push(c);
                }
            }
        }
        ...
        if (recurse) {
            ...
            for (var i = 0; i < children.length; i++) {
                var childRoles = this.getOrganizationRoles(children[i], true);
                for (var j = 0; j < childRoles.length; j++) {
                    list.push(childRoles[j]);
                }
            }
        }
        return list;
    ...
    //==================================================================================================================
    // returns roles that the contributor roles
    getContributorRoles: function (contributorName) {
        // find all this person's roles
        var contributor = this.contributors[contributorName];
        var roleList = [];
        for (var j in this.roles) {
            if (this.roles[j].contributor.toLowerCase() === contributor.name.toLowerCase()) {
                roleList.push(j);
            }
        }
        return roleList;
    ...
    //==================================================================================================================
    // returns an org traversal tree that notes which org node matches the application, with a list of matching roles
    getApplicationSubordinatesTree: function ( applicationName, organizationName ) {
        var newNode = {children: [], matchingRoles: [], name: organizationName};
        ...
        var roles = this.getOrganizationRoles( org, false );
        for (var i = 0; i < roles.length; i++) {
            var matches = this.applicationHasAccountabilitiesInRole( applicationName, roles[i] );
            if (matches) {
                newNode.matchingRoles.push(roles[i]);
            }
        }
    ...
    applicationHasAccountabilitiesInRole: function (applicationName, roleId) {
        var accountabilities = this.roleAccountabilities[roleId];
        if (accountabilities) {
            for (var i = 0; i < accountabilities.length; i++) {
                if (accountabilities[i].application.toLowerCase() === applicationName.toLowerCase())
                {
    ...
    createFilteredMioarchy: function(filterParams) {
        ...
        var roleAccountabilities = deepCopyMap(this.roleAccountabilities, startDate, endDate);
        ...
        var newMioArchy = new Mioarchy(roles, organizations, contributors, applications,
            roles, orgAccountabilities, roleAccountabilities, organizations);
    ...
    buildTimelineEventIndex: function() {
        try {
            ...
            eventLists = concatEventLists(eventLists, getEventList(this.roles));
            eventLists = concatEventLists(eventLists, getEventList(this.roleAccountabilities));
...
// Role 'class'
function Role(id, organization, application, role, accountabilityLabel, accountabilityLevel, contributor, primaryAccountability, startDate, endDate) {
    this.type = Mioarchy.prototype.Types.Role;
    this.id = id;

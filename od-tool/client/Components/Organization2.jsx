...
class RenderingUtilities {
	...
	calculateBoundingCircleDiameter(width, height) {
		return Math.sqrt( width*width + height*height );
	};
}

class RenderInfoCircles {
	constructor(numCircles, minDistanceBetweenCircles, circleDiameter, useMiddle) {
...
class RenderInfoOrganization {
	constructor(org, mioarchy) {
		// constants
		...
		this.determineMaximumSubOrgDimensions();
		...
		if (this.orgLevel > 2 && !this.org.isApplication) {
			this.processCircularSubOrgRendering();
		} else {
			this.processRectangularOrgRendering();
		}
	}

	determineMaximumSubOrgDimensions() {
	...
	processCircularSubOrgRendering() {
	...
	processRectangularOrgRendering() {
		...
		var applicationOrgInfos = {
			positions: [],
			orgRefs: [],
			width: 0,
			height: 0,
			cx: this.MIN_DISTANCE_BETWEEN_CIRCLES
		};
	...
	renderJobsAtThisOrgLevel(x, y, dx, dy) {
		// now move all the circle locations as needed
		var circleLocations = RenderingUtilities.translatePoints(
			this.circleForContributorsAtThisOrgLevel.circleCenters, dx, dy);
	...
	render(x, y) {
		if (this.orgLevel <= 2 || this.org.isApplication) {
			this.renderRectangularOrg(x, y);
		} else {
			this.renderCircularOrg(x, y);
		}
	}

	getJobTopApplicationAccountabilities(roleId, mioarchy, maxAccountabilities) {
		var list = [];
	...
	// render top level organization
	renderRectangularOrg(x, y) {
		...
		if (this.rolesAtThisLevel.length > 0) {
			if (this.subOrgPositions) {
				...
				renderJobsAtThisOrgLevel.call(this, cx, cy, 0, 0);
			}
		}
		//==========================================================================================================
		...
		for (var i = 0; i < this.childRenderingInfos.length; i++) {
			...
			orgRenderInfo.render(x + orgPosition.x, y + orgPosition.y);
		}
		//==========================================================================================================
	...
	// render sub organization
	renderCircularOrg(x, y) {
		let obj = {children: {}};
		...
		if (!this.isLeaf) {
			...
			let circleLocations = this.subOrgCircles.circleCenters;
			...
			for (var i = 0; i < this.childRenderingInfos.length; i++) {
				...
				// render organization internals
				obj.children.push(orgRenderInfo.render(cx, cy));
			}
			;
		}
		//==========================================================================================================
		...
		if (this.isLeaf) {
			obj = this.renderOrgObject(this.org, x, y, this.width, this.height);
	...
	getJobColorStyleString(roleId, mioarchy) {
	...
	renderOrgObject(org, x, y, width, height) {
